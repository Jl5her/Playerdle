...
async function fetchJerseyNumbers(): Promise<Map<string, Map<string, number>>> {
  const url = "https://github.com/nflverse/nflverse-data/releases/download/rosters/roster_2025.csv"
...
function buildTeamOffense(
  ...
  if (!qb || !rb || !te || !wr1 || !wr2 || !wr3 || !lt || !lg || !c || !rg || !rt) {
    ...
    const nullSlots = Object.entries(slots)
      .filter(([, v]) => !v)
      .map(([k]) => k)
...
function loadExisting(outputPath: string): ExistingData | null {
  ...
  try {
    return JSON.parse(readFileSync(outputPath, "utf-8")) as ExistingData
  } catch {
    return null
  }
}
