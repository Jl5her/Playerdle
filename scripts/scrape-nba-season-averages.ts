...
function loadClassicAnswerPoolIds(): Set<string> {
  const __dirname = dirname(fileURLToPath(import.meta.url))
  const answerPoolPath = resolve(__dirname, "..", "packages", "data", "src", "playerdle", "nba", "answer_pool.json")
...
function writeFanaticData(players: FanaticPlayer[], answerPoolIds: string[]) {
  const __dirname = dirname(fileURLToPath(import.meta.url))
  const playersPath = resolve(__dirname, "..", "packages", "data", "src", "playerdle", "nba", "fanatic_players.json")
  const answerPoolPath = resolve(__dirname, "..", "packages", "data", "src", "playerdle", "nba", "fanatic_answer_pool.json")
