...
export default function GuessInput({ onGuess, guessedIds, disabled }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (tokenMatches.length < 5) {
      const tokenIds = new Set(tokenMatches.map((p) => playerId(p)));
      const fuzzyResults = fuse
        .search(trimmed, { limit: 15 })
        .filter(
          (r) => !guessedIds.has(playerId(r.item)) && !tokenIds.has(playerId(r.item))
        )
        .sort((a, b) => a.score! - b.score!) // Sort by relevance (lower score = better match)
        .map((r) => r.item);
      return [...tokenMatches, ...fuzzyResults].slice(0, 8);
    }

    return tokenMatches.slice(0, 8);
