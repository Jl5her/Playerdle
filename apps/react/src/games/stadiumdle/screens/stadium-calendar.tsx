...
const EPOCH = parseDateKey(STADIUM_EPOCH_DATE_KEY)
...
function DayDetail({ puzzle, result, canPlay, onPlay, onViewResults, inProgressCount }: DayDetailProps) {
  return (
...
export default function StadiumCalendar({
  onClose,
  onPlayArchive,
  historyVersion = 0,
  panel = false,
  id,
}: StadiumCalendarProps = {}) {
  ...
  const history = useMemo(() => {
    const map = new Map<string, StadiumResult>()
    for (const r of getStadiumHistory()) map.set(r.date, r)
    return map
  }, [historyVersion])

  const inProgressDates = useInProgressDates("playerdle-stadium-state:v1:", [historyVersion])

  const selectedPuzzle = useMemo(() => getStadiumPuzzleByDateKey(selected), [selected])
  ...
  const calendar = (
    <ArchiveCalendar
      title="Stadiumdle Archive"
      onClose={panel ? undefined : onClose}
      onBack={panel || onClose ? undefined : () => navigate("/stadiumdle")}
      panel={panel}
      epoch={EPOCH}
      history={history}
