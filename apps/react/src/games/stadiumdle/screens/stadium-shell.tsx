...
import StadiumCalendar from "./stadium-calendar"
import StadiumGame, { type StadiumGameMode } from "./stadium-game"
import StadiumHowToPlay, { stadiumTutorialSeenKey } from "./stadium-how-to-play"
import StadiumStatsOverlay from "./stadium-stats-overlay"
...
type StadiumPanel = "guide" | "stats" | "calendar"

export default function StadiumShell({ screen }: Props) {
  const navigate = useNavigate()
  const location = useLocation()
  const initialShowStats = Boolean((location.state as { showStats?: boolean } | null)?.showStats)
  const panels = usePanelStack<StadiumPanel>(initialShowStats ? "stats" : undefined)
  ...
  useEffect(() => {
    document.title = "Playerdle Stadiumdle"
  }, [])

  useEffect(() => {
    if (screen !== "daily") return
    if (initialShowStats) return
    if (localStorage.getItem(stadiumTutorialSeenKey)) return
    panels.push("guide")
    trackPanelOpened({
      panel: "guide",
      game: "stadiumdle",
  ...
  const mode: StadiumGameMode = screen === "arcade" ? "arcade" : "daily"
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
            {!panels.isAnyOpen && (
              <button
                onClick={() => {
                  panels.push("stats")
                  trackPanelOpened({ panel: "stats", game: "stadiumdle", mode: activeMode })
                }}
            ...
            {!panels.isAnyOpen && (
              <button
                onClick={() => {
                  panels.push("guide")
                  trackPanelOpened({ panel: "guide", game: "stadiumdle", mode: activeMode })
                }}
