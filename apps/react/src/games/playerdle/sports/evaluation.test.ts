...
function makeColumn(key: string, evaluator: SportColumn["evaluator"]): SportColumn {
  return {
    id: key,
    label: key,
    key,
...
describe("evaluateColumn — match evaluator", () => {
  ...
  it("returns incorrect for null vs non-null", () => {
    const result = evaluateColumn(makePlayer({ team: null }), makePlayer({ team: "Eagles" }), col)
    expect(result.status).toBe("incorrect")
...
describe("evaluateColumn — comparison evaluator", () => {
  ...
  it("returns correct when values are equal", () => {
    const result = evaluateColumn(makePlayer({ age: 28 }), makePlayer({ age: 28 }), col)
    expect(result.status).toBe("correct")
    expect(result.arrow).toBeUndefined()
  })

  it("returns close when within threshold", () => {
    const result = evaluateColumn(makePlayer({ age: 26 }), makePlayer({ age: 28 }), col)
    expect(result.status).toBe("close")
  })

  it("returns close exactly at threshold boundary", () => {
    const result = evaluateColumn(makePlayer({ age: 30 }), makePlayer({ age: 28 }), col)
    expect(result.status).toBe("close")
  })

  it("returns incorrect when beyond threshold", () => {
    const result = evaluateColumn(makePlayer({ age: 25 }), makePlayer({ age: 28 }), col)
    expect(result.status).toBe("incorrect")
  })
  ...
  it("shows down arrow when guess is above answer", () => {
    const result = evaluateColumn(makePlayer({ age: 31 }), makePlayer({ age: 28 }), col)
    expect(result.arrow).toBe("↓")
  })

  it("returns incorrect and no arrow for non-numeric values", () => {
    const result = evaluateColumn(makePlayer({ age: "N/A" }), makePlayer({ age: 28 }), col)
    expect(result.status).toBe("incorrect")
    expect(result.arrow).toBeUndefined()
  })

  it("omits arrow when showDirection is false", () => {
    const noArrowCol = makeColumn("age", {
      type: "comparison",
      closeWithin: 2,
      showDirection: false,
    })
    const result = evaluateColumn(makePlayer({ age: 25 }), makePlayer({ age: 28 }), noArrowCol)
    expect(result.arrow).toBeUndefined()
  })
})
