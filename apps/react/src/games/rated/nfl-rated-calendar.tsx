...
const EPOCH = parseDateKey(NFL_RATED_EPOCH)
...
export default function NflRatedCalendar({ panel = false, id, historyVersion = 0 }: Props) {
  ...
  const history = useMemo(() => {
    const map = new Map<string, NflRatedResult>()
    for (const r of getNflRatedHistory()) map.set(r.date, r)
    return map
  }, [historyVersion])

  const inProgressDates = useInProgressDates("playerdle-nfl-rated-state:nfl:", [historyVersion])

  const selectedPuzzle = useMemo(() => getNflRatedPuzzleByDateKey(selected), [selected])
  ...
  function handlePlay() {
    if (selected === todayKey) {
      ...
      navigate(`/rated/archive/${selected}`)
    }
  }
  ...
  const calendar = (
    <ArchiveCalendar
      title="Rated Archive"
      onBack={panel ? undefined : () => navigate("/rated")}
      panel={panel}
      epoch={EPOCH}
      history={history}
