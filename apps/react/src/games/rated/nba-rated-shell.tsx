...
import NbaRatedCalendar from "./nba-rated-calendar"
import NbaRatedGame, { type NbaRatedGameMode } from "./nba-rated-game"
...
type NbaRatedPanel = "how-to-play" | "stats" | "calendar"
...
function HowToPlayPanel() {
  return (
    <div className="flex-1 overflow-y-auto px-5 py-4 text-sm text-primary-800 dark:text-primary-100 space-y-4">
      <p>
        <strong>Rated</strong> shows the NBA 2K26 OVR ratings of an NBA team's starting lineup on a
        half-court. Guess which team it is in <strong>5 tries</strong>. Tap any badge to see the
        player's name.
      </p>
...
export default function NbaRatedShell({ screen, archiveDateKey }: Props) {
  const navigate = useNavigate()
  const panels = usePanelStack<NbaRatedPanel>()
  const isArchive = !!archiveDateKey
  const mode: NbaRatedGameMode = screen === "arcade" ? "arcade" : "daily"
  ...
  useEffect(() => {
    document.title = "Rated · NBA"
  }, [])

  useEffect(() => {
    if (screen !== "daily" || isArchive) return
    const seen = localStorage.getItem("nba-rated-tutorial-seen")
    if (!seen) {
      panels.push("how-to-play")
      localStorage.setItem("nba-rated-tutorial-seen", "true")
    }
  }, [screen, isArchive])
