...
import NflRatedCalendar from "./nfl-rated-calendar"
import NflRatedGame, { type NflRatedGameMode } from "./nfl-rated-game"
...
type NflRatedPanel = "how-to-play" | "stats" | "calendar"
...
function HowToPlayPanel() {
  return (
    <div className="flex-1 overflow-y-auto px-5 py-4 text-sm text-primary-800 dark:text-primary-100 space-y-4">
      <p>
        <strong>Rated</strong> shows the Madden 26 OVR ratings of an NFL team's starting lineup in
        a shotgun formation. Guess which team it is in <strong>5 tries</strong>. Tap any badge to
        reveal the player's name.
      </p>
      ...
      <div className="space-y-2">
        ...
        <ul className="space-y-1">
          <li>
            ...
            <strong>Green</strong> — Within 7 OVR of the answer at that position
          </li>
          <li>
            <span className="inline-block w-4 h-4 rounded-full bg-gray-600 align-middle mr-1" />
            <strong>Gray</strong> — More than 7 OVR difference at that position
          </li>
        </ul>
      </div>
    </div>
...
export default function NflRatedShell({ screen, archiveDateKey }: Props) {
  const navigate = useNavigate()
  const panels = usePanelStack<NflRatedPanel>()
  const isArchive = !!archiveDateKey
  const mode: NflRatedGameMode = screen === "arcade" ? "arcade" : "daily"
  ...
  useEffect(() => {
    document.title = "Rated · NFL"
  }, [])

  useEffect(() => {
    if (screen !== "daily" || isArchive) return
    const seen = localStorage.getItem("nfl-rated-tutorial-seen")
    if (!seen) {
      panels.push("how-to-play")
      localStorage.setItem("nfl-rated-tutorial-seen", "true")
    }
  }, [screen, isArchive])

  const stats = calculateRatedStats()
