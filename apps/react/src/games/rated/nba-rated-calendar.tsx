...
const EPOCH = parseDateKey(NBA_RATED_EPOCH)
...
export default function NbaRatedCalendar({ panel = false, id, historyVersion = 0 }: Props) {
  ...
  const history = useMemo(() => {
    const map = new Map<string, NbaRatedResult>()
    for (const r of getNbaRatedHistory()) map.set(r.date, r)
    return map
  }, [historyVersion])

  const inProgressDates = useInProgressDates("playerdle-nba-rated-state:nba:", [historyVersion])

  const selectedPuzzle = useMemo(() => getNbaRatedPuzzleByDateKey(selected), [selected])
  ...
  function handlePlay() {
    if (selected === todayKey) {
      ...
      navigate(`/nba/rated/archive/${selected}`)
    }
  }
  ...
  const calendar = (
    <ArchiveCalendar
      title="Rated Archive"
      onBack={panel ? undefined : () => navigate("/nba/rated")}
      panel={panel}
      epoch={EPOCH}
      history={history}
