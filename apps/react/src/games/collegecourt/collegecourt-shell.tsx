...
import CollegeCourtCalendar from "./collegecourt-calendar"
import CollegeCourtGame, { type CollegeCourtGameMode } from "./collegecourt-game"
...
type CollegeCourtPanel = "how-to-play" | "stats" | "calendar"
...
export default function CollegeCourtShell({ screen, archiveDateKey }: Props) {
  const navigate = useNavigate()
  const panels = usePanelStack<CollegeCourtPanel>()
  const isArchive = !!archiveDateKey
  const mode: CollegeCourtGameMode = screen === "arcade" ? "arcade" : "daily"
  ...
  useEffect(() => {
    ...
    const seen = localStorage.getItem("collegecourt-tutorial-seen:nba")
    if (!seen) {
      panels.push("how-to-play")
      localStorage.setItem("collegecourt-tutorial-seen:nba", "true")
    }
  }, [screen, isArchive])

  const stats = calculateCollegeCourtStats()
